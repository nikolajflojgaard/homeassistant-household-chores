...
- `household_chores.create_task`
  - input: optional `entry_id`, required `title`, optional `date` (`YYYY-MM-DD`), optional `slot` (`am|pm`), optional `assignees[]`, optional `assignee_names[]`
...
- `household_chores.update_task`
  - input: optional `entry_id`, match via `task_id` or `title` + optional `date` + optional assignees, then update with `new_title`, `new_date`, optional `new_slot` (`am|pm`), `new_assignees[]`, `new_assignee_names[]`
